...
blocks.csv & bitcoin_blocks_pool.parquet   pools.json
      ...
      ├──────────── update_metrics.py ───────┼──────── process_forensics.py ────┐
              ...
              ┌───────── browser ───────┘
                    ...
                    └── loadData() returns  → { blocks, poolMeta, poolsInfo, timelines, ecosystem, slugToName }
                          │
...
**`data/processed/blocks_post_2021.parquet`** — 3.3 MB
**`data/processed/blocks_pre_2021.parquet`**  — 7.6 MB
```
height      : int32      — block height
...
### Inputs
It reads the `blocks_post_2021.parquet` generated by the primary pipeline.

### Pool Coverage — Two Thresholds

The script defines two pool lists with different coverage goals:

| Variable | Threshold | Used in | Purpose |
|---|---|---|---|
| `top_pools` | > 1% all-time hashrate | KPI 3 (entropy heatmap), KPI 4 (sync histogram), KPI 7 (BIP 110) | Visual charts — keep clean, ≤ 13 pools |
| `extended_pools` | All identified pools (with ≥ 50 blocks) | KPI 5 (empty block %), KPI 6 (block density) | Lookup table data — cover every active pool regardless of history length |

The distinction matters for newer pools like **Ocean** (Dec 2023) or **Slushpool** (Nov 2024)
that have > 1% current share but a short post-2021 history. They appear in `extended_pools`
so the Top Miners table shows their empty-block % — but they are excluded from the heatmap
and sync charts to avoid visual clutter.

Each KPI loop still guards with `if len(p_df) < 50: continue` to reject truly tiny pools.

### Transformations applied:
1. **Consecutive Block Streaks** (KPI 1): Finds sequences where the same pool mined $N$ continuous blocks. High $N$ correlates with 2-block reorg risks.
2. **Block Interval Latency / Sync Histogram** (KPI 4): Calculates time deltas between consecutive blocks by the same pool. Bins into latency buckets (`< 30s`, `30s–60s`). `< 30s` is the danger zone for network forks.
3. **Luck Funnel / Z-Score** (KPI 2): Groups by day to find actual production vs expected probability based on global market share. Calculates rolling variance and Z-scores to highlight statistically improbable luck (hidden hashrate).
4. **Miner Timing Patterns / Entropy** (KPI 3): Calculates the Coefficient of Variation (CV) on block arrival deltas to categorise operators as Corporate (Low Entropy) vs Retail (High Entropy).
5. **Empty Block Auditor** (KPI 5): Per-pool empty block count and ratio at all-time and 30-day horizons. Uses `extended_pools`.
6. **Block Density Index** (KPI 6): Average transaction count and byte density per pool. Uses `extended_pools`.
7. **BIP 110 Battleground** (KPI 7): Signaling trend, efficiency scatter, and overhead bar per pool.

### Outputs
**`dashboard/data/forensics_data.json`**
Top-level keys: `kpi1_strikes`, `kpi2_funnel`, `kpi3_entropy`, `kpi4_sync`,
`kpi5_empty_blocks` (leaderboard + monthly_trend), `kpi6_density`, `kpi7_bip110`, `last_updated`.

**kpi5 leaderboard entry shape:**
```json
{ "pool": "ocean", "total_all": 746, "empty_all": 4, "ratio_all": 0.54,
  "total_30d": 52, "empty_30d": 0, "ratio_30d": 0.0 }
```
The `pool` field is the raw parquet slug (e.g. `"ocean"`), **not** the display name.
The browser resolves it to a display name via `slugToName` before building the lookup map.
...
`lookup` (`lookup_slug_to_name.json`) is the canonical slug → display name map.
It is used in step 3 to resolve `pool_name`, and is also returned as `slugToName`
in the dataset so callers (e.g. `renderTopMinersTable`) can do reverse lookups
without re-inventing normalisation.
